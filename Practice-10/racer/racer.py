...
try:
    pygame.mixer.music.load("suppliers/background.wav")
...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.transform.scale(pygame.image.load("suppliers/coin.png").convert_alpha(), (150, 150))
        self.rect = self.image.get_rect()
        self.mask = pygame.mask.from_surface(self.image)
        self.respawn()
...
while True:
    ...
    if pygame.sprite.spritecollideany(P1, enemies, pygame.sprite.collide_mask):
        ...
        try:
            pygame.mixer.Sound("suppliers/crash.wav").play()
        except:
            pass
        time.sleep(0.1)
        screen.fill(red)
